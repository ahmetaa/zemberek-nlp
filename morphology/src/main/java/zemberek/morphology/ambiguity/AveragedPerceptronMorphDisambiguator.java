...
public class AveragedPerceptronMorphDisambiguator extends AbstractDisambiguator {
  ...
  public void test(File testFile) throws IOException {
    ...
    System.out.println(
        "total:" + total + " hit=" + hit + String.format(" Accuracy:%f", (double) hit / total));
  }
  ...
  private void updateAverageWeights(int numExamples, String feat) {
    ...
    averagedWeights.put(
        feat,
        (averagedWeights.weight(feat) * featureCount + (numExamples - featureCount) * weights
            .weight(feat))
            / numExamples);
...
/*        if (w3.all.contains(".+Punc") && w3.igs.contains("Verb"))
  ...
  class StateId {
    ...
    public boolean equals(Object o) {
      ...
      if (!first.equals(stateId.first)) {
        return false;
      }
      if (!second.equals(stateId.second)) {
        return false;
      }
