...
import smoothnlp.core.io.LineIterator;
...
public class AmbiguityStats extends BaseParser {
  ...
  public void ambiguousGroupStats(String filename) throws IOException {
    ...
    CountingSet<String> uniques = new CountingSet<String>(1000000);
    Map<String, CountingSet<String>> ambiguityGroups = Maps.newHashMap();
    int total = 0;
    for (String line : lines) {
      for (String s : splitter.split(line)) {
        ...
        if (results.size() > 1) {
          ...
          uniques.add(key);
          CountingSet<String> members = ambiguityGroups.get(key);
          if (members == null) {
            members = new CountingSet<String>();
    ...
    for(String s : uniques.getSortedList()) {
      ...
      if (st.overCutoff(count)) {
        ...
        CountingSet<String> members = ambiguityGroups.get(s);
