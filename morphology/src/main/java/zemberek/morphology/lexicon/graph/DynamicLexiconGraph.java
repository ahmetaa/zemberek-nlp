...
public class DynamicLexiconGraph {
  ...
  private boolean expectationsMatches(SuffixSurfaceNode surfaceNode,
      SuffixSurfaceNode surfaceNodeInSuccessor) {
    if (surfaceNodeInSuccessor.isNullMorpheme()) {
      return true;
    }
    if ((surfaceNode.expectations.contains(PhoneticExpectation.ConsonantStart)
        && surfaceNodeInSuccessor.attributes.contains(PhoneticAttribute.FirstLetterConsonant)) ||
        (surfaceNode.expectations.contains(PhoneticExpectation.VowelStart)
            && surfaceNodeInSuccessor.attributes.contains(PhoneticAttribute.FirstLetterVowel))) {
      return true;
    } else {
      return false;
    }
  ...
  private SuffixSurfaceNode addOrReturnExisting(SuffixForm suffixForm,
      SuffixSurfaceNode newSurfaceNode) {
