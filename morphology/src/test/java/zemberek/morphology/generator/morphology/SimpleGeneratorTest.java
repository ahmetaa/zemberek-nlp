...
public class SimpleGeneratorTest {
  ...
  public void regenerateTest() throws IOException {
    ...
    List<String> parseables = SimpleTextReader
        .trimmingUTF8Reader(new File(Resources.getResource("parseable.txt").getFile()))
        .asStringList();
  ...
  public void morphemeGenerationTest() throws IOException {
    ...
    WordAnalyzer parser = new WordAnalyzer(graph);
    SimpleGenerator generator = new SimpleGenerator(graph);
    List<String> testLines = SimpleTextReader
        .trimmingUTF8Reader(new File(Resources.getResource("separate-morphemes.txt").getFile()))
        .asStringList();
    ArrayListMultimap<String, String> results = ArrayListMultimap.create(100, 2);
    for (String testLine : testLines) {
      for (String s : Splitter.on(",").trimResults()
          .split(Strings.subStringAfterFirst(testLine, "="))) {
    ...
    for (String parseable : results.keySet()) {
      ...
      for (WordAnalysis parseResult : parseResults) {
        String[] res = generator
            .generateMorphemes(parseResult.dictionaryItem, parseResult.getSuffixes());
