...
public class ArpaToSmoothLmConverter {
    ...
    public ArpaToSmoothLmConverter(File arpaFile, File lmFile, File tempDir) {
        this(arpaFile, lmFile, tempDir, "utf-8");
    }

    public ArpaToSmoothLmConverter(File arpaFile, File lmFile, File tempDir, String inputEncoding) {
    ...
    public void convertSmall(File binaryLmDir, NgramDataBlock block) throws IOException {
        convert(binaryLmDir, block, SmoothLm.MphfType.SMALL, -1);
    }

    public void convertLarge(File binaryLmDir, NgramDataBlock block, int chunkBits) throws IOException {
        convert(binaryLmDir, block, SmoothLm.MphfType.LARGE, chunkBits);
    }
    ...
    public static class NgramDataBlock {
        ...
        public NgramDataBlock(int fingerPrintSize, int probabilitySize, int backoffSize) {
            this.fingerPrintSize = fingerPrintSize;
            this.probabilitySize = probabilitySize;
            this.backoffSize = backoffSize;
            if (fingerPrintSize == 4) {
                this.fingerprintMask = 0xffffffff;
            } else
                this.fingerprintMask = (1 << fingerPrintSize * 8) - 1;
        }
