package zemberek.morphology.old_analysis;
...
public class WordAnalyzerTest {
  ...
  public void testVowelDrop() {
    DynamicLexiconGraph graph = getLexiconGraph("ağız [A: LastVowelDrop]",
        "nakit [A:LastVowelDrop]", "vakit [A:LastVowelDrop, NoVoicing]");
    assertHasParses(graph, "vakitlere", "ağza", "ağız", "ağızlar", "nakit", "nakitlere", "nakde",
        "vakit", "vakte");
    assertUnParseable(graph, "ağz", "ağıza", "ağzlar", "nakd", "nakt", "nakite", "nakda", "vakide",
        "vakda", "vakite", "vakt");
  }
  ...
  public void testCompounds() {
    ...
    assertHasParses(graph, "zeytinyağım", "zeytinyağına", "zeytinyağı", "zeytinyağcık",
        "zeytinyağcığa", "zeytinyağlarım");
    assertUnParseable(graph, "zeytinyağıcık", "zeytinyağılar");
  }

  @Test
  public void testCompoundsVoicing() {
    DynamicLexiconGraph graph = getLexiconGraph("at [A:NoVoicing]", "kuyruk",
  ...
  static class NounSuffixes extends DynamicSuffixProvider {
    ...
    SuffixForm A3pl_Comp_lAr = getForm("A3pl_Comp_lAr", new Suffix("A3pl"), "lAr",
    ...
    SuffixFormTemplate Noun_Deriv = getTemplate("Noun2Noun", Noun_Root,
    ...
    NounSuffixes() {
      ...
      Noun_TEMPLATE.indirectConnections
          .add(P1sg_Im, Pnon_TEMPLATE, Nom_TEMPLATE, Dat_yA, Dat_nA, Dim_CIK, Noun_Deriv);
      ...
      A3sg_TEMPLATE.indirectConnections.add(Nom_TEMPLATE, Dat_yA, Dat_nA, Noun_Deriv)
          .add(Noun_Deriv.allConnections());
      ...
      registerForms(Noun_Default, A3pl_lAr, P1sg_Im, Dat_yA, Dat_nA, Dim_CIK, Noun_Comp_P3sg_Root,
          Noun_Comp_P3sg, A3pl_Comp_lAr);
        ...
        switch (item.primaryPos) {
          ...
          default:
            throw new UnsupportedOperationException(
                "In this class only some noun morphemes exist.");
        }
      } else {
        switch (item.primaryPos) {
          case Noun:
            NullSuffixForm nullForm = generateNullFormFromTemplate(Noun_TEMPLATE,
                successorConstraint);
