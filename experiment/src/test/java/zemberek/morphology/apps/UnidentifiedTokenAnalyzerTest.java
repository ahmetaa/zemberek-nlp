...
public class UnidentifiedTokenAnalyzerTest {
  ...
  public void shouldParseSmallCaseProperNounsWithSingleQuote() throws IOException {
    ...
    TurkishMorphology parser = TurkishMorphology.builder()
        .addTextDictionaryResources("dev-lexicon.txt").build();
    UnidentifiedTokenAnalyzer uiParser = new UnidentifiedTokenAnalyzer(parser);
    ...
    Assert.assertEquals("[(Ankara:ankara) (Noun,Prop;A3sg+Pnon+Dat:ya)]",
        results.get(0).formatLong());
    ...
    Assert.assertEquals(1, results.size());
    Assert
        .assertEquals("[(Karaman:karaman) (Noun,Prop;A3sg+Pnon+Nom)]", results.get(0).formatLong());

    results = uiParser.analyze("karaman'a");
    Assert.assertEquals(1, results.size());
    Assert.assertEquals("[(Karaman:karaman) (Noun,Prop;A3sg+Pnon+Dat:a)]",
        results.get(0).formatLong());
