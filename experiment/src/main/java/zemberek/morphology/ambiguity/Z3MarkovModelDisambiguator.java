...
/**
 * This implementation is based on "Dilek Z. Hakkani-Tür, Kemal Oflazer and Gökhan Tür Statistical
 * Morphological Disambiguation for Agglutinative Languages Computers and the Humanities 36:
 * 381–410, 2002." paper. This is the modified implementation of the Model-A system described in the
 * paper. Model-A basically uses 3-gram root and multiplication of current IG's (Inflectional Group)
 * with previous two last IG probabilities. Our implementation attaches POS data to root and in some
 * cobditions to IG data Simple Viterbi decoding is utilized for finding the best parse. A
 * predefined penalty is applied to unknown word probabilities. Kneser-Ney Lm generation is done
 * with BerkeleyLm library Language model compression and fast random access is provided via
 * SmoothLm library.
 */
public class Z3MarkovModelDisambiguator extends Z3AbstractDisambiguator implements
    TurkishMorphDisambiguator {
  ...
  public Z3MarkovModelDisambiguator(InputStream rootLmStream, InputStream igLmStream)
      throws IOException {
  ...
  public void test(File testFile) throws IOException {
    ...
    if (sw.elapsed(TimeUnit.MILLISECONDS) > 0) {
      System.out.println(
          "Approximate performance: " + (1000L * total / sw.elapsed(TimeUnit.MILLISECONDS))
              + " per second.");
    }
  }
    ...
    for (int i = 2; i < ambiguousSeq.length; i++) { // we skip the two <s> words.
      ...
      for (int parseIndex = 0; parseIndex < word.size(); parseIndex++) {
        ...
        for (Hypothesis hyp : hypotheses) {
          ...
          double rootLmScore =
              containsUnknown(rootTrigram) ? UNKNOWN_PROB : rootLm.getProbability(rootTrigram);
          double igLmScore =
              containsUnknown(igTrigram) ? UNKNOWN_PROB : igLm.getProbability(igTrigram) * 2;
        ...
        newHypotheses[parseIndex] = new Hypothesis(bestHyp, bestScore, bestHyp.oneBeforeIndex,
            parseIndex);
      }
