...
/**
 * A suffix form represents a lexical form of a suffix and it's connections to other forms. For
 * example for Aorist Tense there are 3 Suffix Forms available. -Ar -Ir -z For all those lexical
 * forms a SuffixForm is used representing the form and the connections before compiling the Lexicon
 * Graph.
 */
public class SuffixForm {
  ...
  public boolean equals(Object o) {
    ...
    if (!generation.equals(that.generation)) {
      return false;
    }
    if (!suffix.equals(that.suffix)) {
      return false;
    }

    if (terminationType != that.terminationType) {
      return false;
    }
    ...
    if (!indirectConnections.equals(that.indirectConnections)) {
      return false;
    }
