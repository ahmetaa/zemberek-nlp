...
public class WordAnalyzer {
  ...
  public List<WordAnalysis> analyze(String input) {
    ...
    for (StemNode candidate : candidates) {
      ...
      initialTokens.add(
          new Token(candidate, Lists.newArrayList(candidate.getSuffixRootSurfaceNode()), rest));
    }
  ...
  static class Token {
    ...
    public String asParseString() {
      StringBuilder sb = new StringBuilder(
          "[" + stemNode.surfaceForm + ":" + getLemma() + "-" + getDictionaryItem().primaryPos
              + "]");
      sb.append("[");
      int i = 0;
      for (SuffixSurfaceNode suffixSurfaceNode : surfaceNodeHistory) {
        sb.append(suffixSurfaceNode.getSuffixForm().getSuffix()).append(":")
            .append(suffixSurfaceNode.surfaceForm);
