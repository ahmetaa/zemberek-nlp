 ...
 * 381–410, 2002." paper. This is the exact implementation of the Model-A system described in the
 * paper. Model-A basically uses 3-gram root and multiplication of current IG's (Inflectional Group)
 * with previous two last IG probabilities. Simple Viterbi decoding is utilized for finding the best
 * parse. A predefined penalty is applied to unknown word probabilities. Kneser-Ney Lm generation is
 * done with BerkeleyLm library Language model compression and fast random access is provided via
 * SmoothLm library.
 */
    ...
    for (SentenceData sentence : testSet.sentences) {
      ...
      for (int parseIndex : bestSeq) {
        ...
        if (wordData.allParses.get(parseIndex).equals(wordData.correctParse)) {
          ...
          System.out.println("miss:" + wordData.word + " Correct:" + wordData.correctParse + " : "
    ...
    for (SentenceAnalysis.Entry entry : sentence) {
      ...
      for (WordAnalysis parse : entry.parses) {
        ...
        for (int k = 0; j < parse.inflectionalGroups.size(); k++) {
          igs[j][k] = igLm.getVocabulary()
              .indexOf(parse.inflectionalGroups.get(k).formatNoSurface());
        }
        j++;
      }
